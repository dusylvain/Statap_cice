...
kpss.test(serietemp)
```
Ici H_0 est : 'stationnaire' : la p-value est inférieure à 0.01%, on rejette donc l'hypothèse nulle (celle-ci) à 1%.
...
Il faut au plus deux retards pour rendre ces séries stationnaires, quel que soit le test considéré.


```{r}
multi_stat_tests(diff(diff(prepadonnees(compte, 'VA', 'TOT'))))#le PIB
multi_stat_tests(diff(diff(prepadonnees(compte, 'EMP', 'TOT'))))#le niveau d'emploi
```
...
```{r}
ligne_pays_bis<-function(base,vari,codei,pays,variable='var',paysbis,varbis,echelle=1)#cette fonction permet d'utiliser à la fois des données de la Banque mondiale et de EU Klems
{
tableau=subset(base,var==vari& code==codei)[c("country", lignedata)] 
rownames(tableau)<-tableau[,1]
...
ligne_pays_bis( compte, vari='VA', codei='TOT', pays=c("FR", "DE", "UK", "ES", "IT"), variable='Valeur ajoutée', varbis="GDP", paysbis=c("AGO","ALB",'FRA'), echelle=10**6 )
```
Attention, les données WB sont en milliards et les EU Klems en billions ! (en cours de changement)
