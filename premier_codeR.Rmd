...
adf.test(serietemp)
```
Ici H_0 est : 'non stationnaire' : la p-value est de 96%, on rejette donc l'hypothèse nulle (celle-ci) à 5%.
...
```
Ici H_0 est : 'non stationnaire' : la p-value est de 99%, on rejette donc l'hypothèse nulle (celle-ci) à 1%.
Donc, la série est stationnaire. 
#Test KPSS
```{r}
kpss.test(serietemp)
```
Ici H_0 est : 'stationnaire' : la p-value est de 0.01%, on ne rejette donc pas l'hypothèse nulle (celle-ci) à 1%.
Donc, la série est stationnaire. 
...
multi_stat_tests<- function(tableau){
    ...
    df_multi$kpss <- df_multi$kpss.pvalue > 0.95
...
```{r}
ligne_pays( compte, vari="EMP", codei='TOT',pays=c("BG") )+ ggtitle("évolution de l'emploi dans les grands pays européens") +
  xlab("année") + ylab("nombre d'emplois en milliers")
```
