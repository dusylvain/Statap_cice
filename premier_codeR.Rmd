...
multi_stat_tests<- function(tableau){
    ...
    df_multi$nb_stationnarite <- sapply(tableau, function(x) forecast::ndiffs(x[!is.na(x)], test = "kpss"))#on regarde le nombre de retards nécessaires pour que la série soit stationnaire
    df_multi
}
```
```{r}
