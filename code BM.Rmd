...
```{r}
graph_stratBM=function(df.melt=df_BM, liste_value='gdb_hab', pays='FRA', date0=1997, date1=2005, date2=2012, date3=2016, synthdon=TRUE, synth.out=0){
  valeur=liste_value[1]
  #on récupère les poids
  ind_pays=as.numeric(df.melt[df.melt$Country.Code==pays,]$id_country[1])
...
newrow=data.frame(valeur=df.melt[df.melt$id_country==a & df.melt$year==2016,][,valeur]*solpoids[1], index=1,country=unique(df.melt[df.melt$id_country==a,]$Country.Code))
dfstrat=newrow

for(poids in solpoids){
  if(a>1 & a!=ind_pays){
   newrow=data.frame(valeur=df.melt[df.melt$id_country==a & df.melt$year==2016,][,valeur]*poids, index=a,country=unique(df.melt[df.melt$id_country==a,]$Country.Code))
...
newrow=df.melt[df.melt$id_country==numgrand[1] & df.melt$year>date0-1 & df.melt$year<date3+1,][,valeur]*solpoids[numgrand[1]]
newdata=data.frame(valeur=newrow, country=unique(dfstrat[dfstrat$index==numgrand[1],]$country), temps=date0:date3)
dfstratfin=newdata

for(pays in 2:(max(df.melt$id_country)-2)){
   newrow=newrow+df.melt[df.melt$id_country==numgrand[pays]& df.melt$year>date0-1 & df.melt$year<date3+1,][,valeur]*solpoids[numgrand[pays]]
   if (pays<6){
   ...
   if(pays==max(df.melt$id_country)-2){
