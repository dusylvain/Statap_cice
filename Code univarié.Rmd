...
Lignes 250 à 400 : fonction qui effectue le contrôle synthétique univarié
Lignes 400 à 530 : Calcul des RMSPE
Lignes 530 et plus : travail en cours sur du brouillon
```{r}
library(ggplot2)
library(reshape2)
library(dplyr)
...
```
#Nouvelle fonction gen_liste plus adaptable et qui évite l'overfitting !
```{r}
gen_liste2=function(liste_value, date=c(list(2000:2007), list(2008:2011), 2012))
{
a=0
...
```{r}
univariate_synth=function(liste_value, df_synth=df.melted_i, pays='FRA', date_opti_inf = 2000,date_opti_sup = 2012, ordonnee = NA, date_plot_inf = 1998, date_plot_sup = 2016, nb_country_plot = 5, colorgraph = TRUE){
...
#ON COMMENCE PAR RECUPERER, POUR CHAQUE PAYS, SA CONTIBUTION (valeur*poids)
for (countries in unique(poids1$country)){
        for (i in 1:length(unique(df_synth$country))){ 
          ...
          if (countries == df_synth$country[i] & i <ind_pays){
            for (years in date_plot_inf:date_plot_sup){
...
for (years in date_plot_inf:date_plot_sup){#on initialise
...
#On ajoute l'unité synthétique
for (years in date_plot_inf:date_plot_sup){
  newrow = dfstrat[dfstrat$country == unique(dfstrat$country)[1] & dfstrat$temps == years,]$valeur
  for (countries in unique(dfstrat$country)[2:length(unique(dfstrat$country))]){ 
    newrow = newrow + dfstrat[dfstrat$country == countries & dfstrat$temps == years, ]$valeur}
  temp_df = data.frame(valeur=newrow,
                          index=0,
                          country='Synthetic Unit',
                          weight = 1,
                          temps = years)
  dfstratfin = rbind(dfstratfin, temp_df)
}
...
for (years in date_plot_inf:date_plot_sup){
  temp_df = data.frame(valeur=df_synth[df_synth$country == pays & df_synth$temps == years,][,valeur],
                       country= pays,
                       temps=years,
                       weight = 1,
                       index = ind_pays)
...
```{r}
poids = univariate_synth(c('VA_ind'), df_synth=df.evol, date_opti_inf = 2001, date_opti_sup = 2012, pays = 'FRA', date_plot_inf = 2000, date_plot_sup = 2016, nb_country_plot = 5, colorgraph = TRUE)
```
...
#On raajoute l'unité synthétique
for (years in date_plot_inf:date_plot_sup){
  newrow = dfstrat[dfstrat$country == unique(dfstrat$country)[1] & dfstrat$temps == years,]$valeur
...
#group.colors = (unique(dfstratfin[dfstratfin$index==1],]$country)
colorplot = ggplot(data=dfstratfin,aes(x=temps, y=valeur))+
  geom_line(aes(group=country, col=country))#+
