...
tableau=subset(basei,(var==vari)& code==codei)[c("country3L", lignedata,"var")]
...
```{r}
df.melted=df.melted%>% rename(VA=value.x,Emploi=value.y)
```
...
#On enleve les colonne mnt innutile varx et vay
```{r}
df.melted2=df.melted2 %>% select(-c(var.x,var.y))
```

#Intégration de la population
```{r}
df.melted2=df.melted2[c("country",'temps','index','VA','Emploi','W')]
...
```{r}
 dataprep.out=Synth::dataprep(foo=df.melted2,
                predictors = c("VA_pop","Emploi","W"), #Les variables utilisées en contrôle
...
```
```{r}
optix=quiet(calculpoids(df.melted, 'VA'))
```
...
tripleSynth(liste_value=c("VA", "Emploi", "W"), df.melted2, pays='FR')
```
