...
```{r}
df.melted=df.melted%>% rename(VA=value.x,Emploi=value.y,Productivite=value)
```
...
```{r}
df.melted2=df.melted2 %>% select(-c(var.x,var.y,var))
```

#Intégration de la population
```{r}
df.melted2=df.melted2[c("country",'temps','index','VA','Emploi','W','Productivite')]
...
#Intégration du taux de chomage
```{r}
autre_pays
...
```{r}
 dataprep.out=Synth::dataprep(foo=df.melted2,
                predictors = c("VA_pop","Emploi","W","Productivite","chomage"), #Les variables utilisées en contrôle
...
```{r}
variable=c("Invest_pop","W","VA_pop","Productivite","chomage")
dataprep.out=Synth::dataprep(foo=df.melted_i,
                special.predictors = gen_liste(variable),#Les variables utilisées en contrôle
                predictors.op = "mean", #La manière dont on les prend en compte
                time.predictors.prior = 2012, #période pré-traitement
                dependent="Emploi", #variable d'intérêt
                unit.variable = "index", #colonne avec les index des pays
