...
```{r}
graph_strat=function(solpoids=synth.out$solution.w, df.melt=df.melted_i, valeur='VA'){
a=1
poids_pays=data.frame(poids=solpoids[1],country=unique(df.melt[df.melt$index==a,]$country), index=1)
newrow=data.frame(valeur=df.melt[df.melt$index==a,]$VA, temps=1997:2016, index=1)
dfstrat=newrow

for(poids in solpoids){
  if(a>1){
   newrow=newrow+data.frame(valeur=df.melt[df.melt$index==a,]$VA, temps=0, index=1)
...
dataprep.out=Synth::dataprep(foo=df.melted_i,
                ...
                dependent="chomage", #variable d'intérêt
                unit.variable = "index", #colonne avec les index des pays
                unit.names.variable="country", #nom des pays
