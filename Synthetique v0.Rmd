...
```
```{r}
simpleSynth(c('evol_Emp'), df.melt=df.evol, dateplot=1998:2016)
```
...
```{r}
graph_strat=function(df.melt=df.melted_i, liste_value='VA', pays='FRA', date0=1997, date1=2005, date2=2012, date3=2016){
...
poidstri=order(solpoids, decreasing=TRUE)[1:8]
#puis on construit les séries augmentées
##problème : on n'a pas enlevé le pays considéré !
a=1
poids_pays=data.frame(poids=solpoids[1],country=unique(df.melt[df.melt$index==a,]$country), index=1)
newrow=data.frame(valeur=df.melt[df.melt$index==a & df.melt$temps==2016,][,valeur]*solpoids[1], index=1)
dfstrat=newrow

for(poids in solpoids){
  if(a>1 & a!=ind_pays){
   newrow=data.frame(valeur=df.melt[df.melt$index==a & df.melt$temps==2016,][,valeur]*poids, index=a)
...
newrow=df.melt[df.melt$index==numgrand[1],][,valeur]*solpoids[numgrand[1]]
newdata=data.frame(valeur=newrow, country=unique(df.melt[df.melt$index==numgrand[1],]$country), temps=1997:2016)
dfstratfin=newdata

for(pays in 2:(max(df.melt$index)-2)){
   newrow=newrow+df.melt[df.melt$index==numgrand[pays],][,valeur]*solpoids[numgrand[pays]]
   print(newrow)
   if (pays<6){
     newdata=data.frame(valeur=newrow, country=unique(df.melt[df.melt$index==numgrand[pays],]$country), temps=1997:2016)
   dfstratfin=rbind(dfstratfin, newdata) 
   }
   if(pays==max(df.melt$index)-2){
     newdata=data.frame(valeur=newrow, country='autres', temps=1997:2016)
     dfstratfin=rbind(dfstratfin, newdata)
     }
}
