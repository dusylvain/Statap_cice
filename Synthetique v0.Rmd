...
Xt=subset(df.melted, country=='FR'& temps<2013 &temps>1999)
Xt=Xt$value.x#on récupère les données pour la France
Xn=subset(df.melted, temps<2013 &temps>1999)
Xn=matrix(Xn$value.x, nrow=26)
Xn=Xn[c(1:11, 13:26),]#on récupère les autres
```
```{r}
x0=c(rep(1/25,25))#on crée un vecteur de départ au hasard
...
opti$par=matrix(opti$par, nrow=1)#on transforme les poids trouvés en matrice
```


```{r}
 dataprep.out=Synth::dataprep(foo=df.melted,
                predictors = c("value.x"), #Les variables utilisées en contrôle
                predictors.op = "mean", #La manière dont on les prend en compte
...
dataprep.variables.v=opti$par#on remplace le X0 par les poids qu'on a trouvé
...
Ylab = "Emploi", Xlab = "year", Legend = c("France", "synthetic France"), Legend.position = "bottomright")
```
Il y a visiblement des choses à améliorer !
