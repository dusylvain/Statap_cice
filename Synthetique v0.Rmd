...
newrow=data.frame(valeur=df.melt[df.melt$index==a,][,valeur]*solpoids[1], temps=1997:2016, index=1)
dfstrat=newrow
#boucle sur les pays pour agréger
for(poids in solpoids){
  if(a>1 & a!=ind_pays){
   newrow=newrow+data.frame(valeur=df.melt[df.melt$index==a,][,valeur]*poids, temps=0, index=1)
   dfstrat=rbind(dfstrat, newrow) 
   poids_pays=rbind(poids_pays, data.frame(poids=poids, country=unique(df.melt[df.melt$index==a,]$country), index=a))
   }
   if(a==ind_pays){newrow=newrow+data.frame(valeur=rep(0,date3-date0+1), temps=0, index=1)
   print(newrow)
   }
...
dfstrat=left_join(dfstrat, poids_pays)
poids_pays=order(poids_pays, decreasing=TRUE)


ggplot(data=dfstrat,aes(x=temps, valeur)) +
  geom_line(aes(group=index, col=country))  
}
...
a=1
poids_pays=data.frame(poids=solpoids[1],country=unique(df.melt[df.melt$index==a,]$country), index=1)
newrow=data.frame(valeur=df.melt[df.melt$index==a,]$VA*solpoids[1], temps=1997:2016, index=1)
dfstrat=newrow

for(poids in solpoids){
  if(a>1 & a!=ind_pays){
   newrow=newrow+data.frame(valeur=df.melt[df.melt$index==a,]$VA*poids, temps=0, index=1)
   dfstrat=rbind(dfstrat, newrow) 
   poids_pays=rbind(poids_pays, data.frame(poids=poids, country=unique(df.melt[df.melt$index==a,]$country), index=a))
   }
   if(a==ind_pays){newrow=newrow+data.frame(valeur=rep(0,date3-date0+1), temps=0, index=1)
