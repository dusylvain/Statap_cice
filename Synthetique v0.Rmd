...
a=1
newrow=data.frame(valeur=df.melted_i[df.melted_i$index==a,]$VA, temps=1997:2016, index=1)
dfstrat=newrow
for(poids in synth.out$solution.w){
  if(a>1){newrow=newrow+data.frame(valeur=df.melted_i[df.melted_i$index==a,]$VA, temps=0, index=1)
   dfstrat=rbind(dfstrat, newrow) 
  }
  a=a+1
}
ggplot(data=dfstrat,aes(x=temps, valeur)) +
  geom_line(aes(group=index), colour=rainbow(440))}
```


```{r}
dataprep.out=Synth::dataprep(foo=df.melted_i,
                predictors = 'VA', #Les variables utilisées en contrôle
                predictors.op = "mean", #La manière dont on les prend en compte
                time.predictors.prior = 2012, #période pré-traitement
                ...
                controls.identifier = c(1:23)[-11], #n° des unités de contrôles
                time.variable = 'temps', #colonne où se trouve la date
                time.optimize.ssr = 2000:2014, #période de maximisation
