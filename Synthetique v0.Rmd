...
```{r}
variable=c("Invest_pop","W","VA_pop","Productivite","chomage","export","Part_lab")
...
Xn=Xn[Xn$country!=pays,]#on récupère les autres
Xn=matrix(Xn[,value], nrow=pays_autres)
Xn=t(scale(t(Xn)))#on normalise les données par pays(la normalisation se fait par colonne)
Xt=Xt[1:13]
```
...
```{r}
frsynth=t(opti$par)%*%Xn*sd(c(Xt))+mean(c(Xt))
```

```{r}
Xt=subset(df.melted, country==pays)
Xt=Xt[,value]
Xn=subset(df.melted, country!=pays)
Xn=matrix(Xn[,value], nrow=pays_autres)
plot(c(t(opti$par)%*%Xn*sd(c(Xt))+mean(c(Xt))))
plot(Xt)
