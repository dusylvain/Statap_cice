...
```{r}
variable=c("Invest_pop","chomage","VA_pop","Productivite","export","Part_lab","Emploi")
df.melted_i$logemploi=log(df.melted_i$Emploi)

dataprep.out=Synth::dataprep(foo=df.melted_i,
                predictors = variable,
                time.predictors.prior = 2000:2011, #période pré-traitement
                special.predictors = gen_liste(variable),
                dependent="Emploi", #variable d'intérêt
                unit.variable = "index", #colonne avec les index des pays
                unit.names.variable="country", #nom des pays
                treatment.identifier = 10, # n° de l'unité traitée
                ...
                time.plot=2000:2016, ) #période tracée sur le graphique


synth.out <- Synth::synth(data.prep.obj = dataprep.out, method = c("BFGS"))

Synth::path.plot(synth.res = synth.out, dataprep.res = dataprep.out,
Ylab = "PIB", Xlab = "year", Legend = c("France", "synthetic France"), Legend.position = "bottomright",Ylim=c(25000,28000),)
