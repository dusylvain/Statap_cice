...
for (pays in unique(df.melted_i$country)){
a=df.melted_i%>%filter(country==pays)%>%select(temps,country,VA)
a$VA[2:nrow(a)]=diff(a$VA)/a$VA*100
a=a%>%rename(evol_VA=VA)
base=rbind(base,a)
}
df.evol=df.melted_i%>%full_join(base,on=c('country','temps'))
...
```{r}
 dataprep.out=Synth::dataprep(foo=df.melted2,
                ...
                controls.identifier = c(1:10,12:23), #n° des unités de contrôles
...
dataprep.out=Synth::dataprep(foo=df.melted2,
                ...
                treatment.identifier = 10, # n° de l'unité traitée
                controls.identifier = c(1:9,11:21), #n° des unités de contrôles
                time.variable = 'temps', #col*onne où se trouve la date
                time.optimize.ssr = 1999:2012, #période de maximisation
                time.plot=1995:2017) #période tracée sur le graphique
synth.out <- Synth::synth(data.prep.obj = dataprep.out, method = "BFGS")
...
df.melted2
dataprep.out=Synth::dataprep(foo=df.melted2,
                predictors.op = "mean", #La manière dont on les prend en comptz
                time.predictors.prior = 2012, #période pré-traitement
...
```{r}
tripleSynth(liste_value=c("chomage"), df.melted2, pays='FRA')
```
```{r}
tripleSynth(liste_value=c("VA_pop"), df.melted2, pays='FRA')
```
```{r}
tripleSynth(liste_value=c("chomage","VA_pop", "W", "Productivite", "export"), df.melted2, pays='FRA')
```
```{r}
tripleSynth(liste_value=c("VA_pop", "Emploi", "W", "Productivite", "export"), df.melted2, pays='FRA')
```
...
dataprep.out=Synth::dataprep(foo=df.evol,
                predictors = (variable),#Les variables utilisées en contrôle
                time.predictors.prior = c(2000:2011), #période pré-traitement
                special.predictors = gen_liste("evol_VA"),
                dependent="evol_VA", #variable d'intérêt
                unit.variable = "index", #colonne avec les index des pays
                unit.names.variable="country", #nom des pays
                treatment.identifier = 11, # n° de l'unité traitée
                controls.identifier = b, #n° des unités de contrôles
                time.variable = 'temps', #colonne où se trouve la date
...
Synth::path.plot(synth.res = synth.out, dataprep.res = dataprep.out,
Ylab = "PIB", Xlab = "year", Legend = c("France", "synthetic France"), Legend.position = "bottomleft")
...
for (i in c(2:20))
{
  ...
  dataprep_i.out=Synth::dataprep(foo=df.melted_i,
                predictors = (variable),
                special.predictors = list(list("Emploi",2000:2005,"mean"),list("Emploi",2010:2011,"mean")),
...
#Synth::gaps.plot(synth.res = synth.out_i, dataprep.res = dataprep_i.out,Ylab = "PIB", Xlab = "year")
  ...
  if (i==2) { #On initialise une base avec en ligne le noms de l'unité puis l'effet du traitement pour chaque année (le numéro de l'année n'est pas dans la base)
...
```{r}
qplot(x=x, y=valeur,color=noms, data=df_effe, geom="line",ylim=c(-2000,2000))+ xlab("Date")+scale_color_manual(breaks = unique(df2$noms),                        values=couleur)

qplot(x=x, y=valeur,color=noms, data=df_effe, geom="line",ylim=c(-2000,2000))+ xlab("Date")
...
SynthNorm('VA')
```
#fonction essai pour raccourcir la précédante
```{r}
#SynthNorm=function(value, df.melt=df.melted,pays='FRA', date1=1999, date2=2013, date0=1997, date3=2016){
pays_autres=length(c(table(df.melt$country)))-1
Xtout=subset(df.melt)[,c(value, 'country', 'temps')]
Xautres=Xtout[Xtout$country!=pays,][,c(value, 'temps')]
Xpays=Xtout[Xtout$country==pays,][,c(value, 'temps')]
Xn=matrix(Xautres[Xautres$temps<date2,][Xautres$temps>date1,][,value], nrow=pays_autres)
Xnorm=t(scale(t(Xn)))#on normalise les données par pays(la normalisation se fait par colonne)
Xnorm2=t(scale(t(Xautres[,value])))

Xtnorm=scale(Xpays[Xpays$temps<date2 &Xpays$temps>date1,])[1:(date2-date1+1)]#on récupère les données pour la France
...
opti$par=matrix(opti$par, ncol=1)
#plot
FrSynth=t(opti$par)%*%Xnorm*sd(c(Xpays))+mean(c(Xpays))
df=data.frame(c(FrSynth, Xt))
df$X=c(rep(2,nrow(df)/2),rep(3,nrow(df)/2))
df$dat=c(date0:date3, date0:date3)
ggplot(data=df,aes(x=dat, c.FrSynth..Xt., col=X)) +
